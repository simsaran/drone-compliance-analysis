...
with tab2:
    ...
    profile_display["Value"] = profile_display["Value"].astype(str)
...
with tab3:
    ...
    with col2:
        ...
        penalty_data["Penalty Numeric"] = penalty_data["Max Penalty"].str.replace(r'[$,]', '', regex=True).str.extract(r'(\d+)')[0].fillna(0).astype(float)
        fig_pen = px.bar(
            penalty_data,
            x="Risk ID", y="Penalty Numeric",
            color="Risk Rating",
            color_discrete_map=RISK_COLORS,
            text="Max Penalty",
        )
